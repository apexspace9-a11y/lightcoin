...
class AppStore extends ChangeNotifier {
  ...
  Future<void> addMoney(int id, double amount) async { final i=goals.indexWhere((g)=>g.id==id); if(i<0)return; goals[i]=goals[i].copyWith(current:(goals[i].current+amount).clamp(0.0,double.infinity).toDouble()); await _save(); notifyListeners(); }
