...
class AppStore extends ChangeNotifier {
  ...
  Future<void> load() async {
    ...
    if (events.isEmpty && goals.isEmpty && reminders.isEmpty) {
      _seed();
    }
  ...
  void _seed() {
    ...
    events.add(
      CalendarItem(
        ...
        note: '15 phút để chọn 3 ưu tiên quan trọng.',
        category: 'Công việc',
        remind: false,
      ),
    );
    goals.add(
      SavingGoal(
        ...
        target: 30000000,
        current: 6500000,
        deadline: DateTime(now.year + 1, 1, 1),
      ),
    );
    reminders.add(
      ReminderItem(
        id: 30001,
        title: 'Kiểm tra mục tiêu tiết kiệm',
        dateTime: now.add(const Duration(days: 7)),
        note: 'Cập nhật số tiền đã tiết kiệm.',
        enabled: false,
      ),
    );
    unawaited(_save());
  ...
  Future<void> addEvent(CalendarItem item) async {
    events.add(item);
    events.sort((a, b) => a.dateTime.compareTo(b.dateTime));
    await _save();
    notifyListeners();

    if (notifications && item.remind) {
  ...
  Future<void> deleteEvent(CalendarItem item) async {
    events.removeWhere((e) => e.id == item.id);
    notifyListeners();
    await _save();
  ...
  Future<void> addMoney(int id, double amount) async {
    ...
    goals[i] = goals[i].copyWith(
      current: (goals[i].current + amount)
  ...
  Future<void> toggleReminder(
    ...
    final updated = r.copyWith(done: done, enabled: enabled);
    reminders[i] = updated;
    notifyListeners();
    await _save();

    if (updated.enabled && !updated.done && notifications) {
      await NotificationService.instance.schedule(
        id: updated.id,
        title: updated.title,
  ...
  Future<void> _rescheduleAll() async {
    ...
    for (final e in events.where(
      (x) => x.remind && x.dateTime.isAfter(now),
    )) {
      await NotificationService.instance.schedule(
        id: e.id,
        title: e.title,
        body: e.note.isEmpty ? 'Bạn có lịch hẹn sắp tới.' : e.note,
        at: e.dateTime,
      );
    }

    for (final r in reminders.where(
      (x) => x.enabled && !x.done && x.dateTime.isAfter(now),
    )) {
      await NotificationService.instance.schedule(
        id: r.id,
        title: r.title,
